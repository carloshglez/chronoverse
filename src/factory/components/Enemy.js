...
import { randomNumBetween, doExplode, getBulletObject } from '../../util/helpers';
...
export default class Enemy {
	...
	moveAsHunter(state) {
		...
		if (this.gameMode === GAME_MODE.SPACE_RACE) {
	...
	shoot() {
		if (Date.now() - this.lastShot > this.shotFrequency) {
			...
			for (var index = 0; index < bulletArrayDirectionsX.length; index++) {
				...
				const bullet = new Bullet(getBulletObject(this.rotation, this.position, 2, directionValueX, false, this.color, 'enemyBullet'));
