...
import { PW } from './powerUpHelper';
import { AWARDS } from './awardsHelper';
import { GAME_MODE } from './factoryHelper';
...
export function updateInnerStrings() {
    ...
    keyNames.forEach(name => {
        if (name.startsWith("lbAward")) {
            //console.log(i +": " + name + "-->" + strings[name]);
            AWARDS[i].text = strings[name];
            i++;
        }
    });
    var i = 0;
    keyNames.forEach(name => {
        if (name.startsWith("pw")) {
            //console.log(i +": " + name + "-->" + strings[name]);
            PW[Object.keys(PW)[i]].text = strings[name];
            i++;
        }
    });
}